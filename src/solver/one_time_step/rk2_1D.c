...
FLOAT_P rk2_1D(struct BackgroundVariables *bg, struct ForegroundVariables *fg_prev, struct ForegroundVariables *fg, struct GridInfo *grid_info, struct MpiInfo *mpi_info, FLOAT_P dt_last, bool first_timestep)
{
    ...
    */
    ...
    {
        k1_s1[i] = damping_factor[i] * rhs_ds1_dt_1D(bg, fg_prev, grid_info, i);
        k1_vz[i] = damping_factor[i] * rhs_dvz_dt_1D(bg, fg_prev, grid_info, i);
    }

    // Updating fg to hold mid-calculation values
    for (int i = nz_ghost; i < nz_full-nz_ghost; i++)
    {
    ...
    // Extrapolating s1 and vz to ghost cells
    ...
    for (int i = nz_ghost; i < nz_full-nz_ghost; i++)
    {
        fg->s1[i] = fg_prev->s1[i] + 0.5 * dt * (k1_s1[i] + k2_s1[i]);
        fg->vz[i] = fg_prev->vz[i] + 0.5 * dt * (k1_vz[i] + k2_vz[i]);
    }
