    ...
    */
    ...
    for (int i = nz_ghost; i < nz_full - nz_ghost; i++)
    {
        for (int j = 0; j < ny; j++)
        {
            for (int k = 0; k < nx; k++)
            {
                k3_s1[i][j][k] = damping_factor[i] * rhs_ds1_dt_3D(bg, fg, grid_info, i, j, k);
                k3_vx[i][j][k] = rhs_dvx_dt_3D(bg, fg, grid_info, i, j, k);
                k3_vy[i][j][k] = rhs_dvy_dt_3D(bg, fg, grid_info, i, j, k);
                k3_vz[i][j][k] = damping_factor[i] * rhs_dvz_dt_3D(bg, fg, grid_info, i, j, k);
            }
        }
    }
    ...
    for (int i = nz_ghost; i < nz_full-nz_ghost; i++)
    {
        for (int j = 0; j < ny; j++)
        {
            for (int k = 0; k < nx; k++)
            {
                fg->s1[i][j][k] = fg_prev->s1[i][j][k] + dt/6.0 * damping_factor[i]*(k1_s1[i][j][k] + 4.0 * k2_s1[i][j][k] + k3_s1[i][j][k]);
