...
FLOAT_P rhs_elliptic_eq_1D(struct BackgroundVariables *bg, struct ForegroundVariables *fg, struct GridInfo *grid_info, int i)
{
    ...
    */
    ...
    // First derivatives
    FLOAT_P d_vz_dz, d_rho1_dz;

    // Second derivatives
    FLOAT_P dd_vz_ddz;

    #if CENTRAL_ORDER == 2
        d_vz_dz = central_first_derivative_second_order(vz[i-1], vz[i+1], dz);
        d_rho1_dz = central_first_derivative_second_order(rho1[i-1], rho1[i+1], dz);
