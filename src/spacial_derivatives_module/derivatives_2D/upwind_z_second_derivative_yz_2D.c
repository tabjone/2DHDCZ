...
FLOAT_P upwind_z_second_derivative_yz_2D(FLOAT_P **array, FLOAT_P **velocity, int i, int j, int ny, FLOAT_P one_over_2dydz)
{
    ...
    */
    ...
    if (velocity[i][j] >= 0)
    {
        return (array[i][j_plus] - array[i][j-1] - array[i-1][j_plus] + array[i-1][j_minus]) * one_over_2dydz;
    }
    else
    {
        return (array[i+1][j_plus] - array[i+1][j_minus] - array[i][j_plus] + array[i][j_minus]) * one_over_2dydz;
    }
}