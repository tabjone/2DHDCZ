...
{
    /*
    Communicates the ghost cells of the first process to the last process and vice versa for a 2D array.
    ...
    */
    ...
    if (mpi_info->size == 1)
    {
        for (int i = 0; i < nz_ghost; i++)
        {
            for (int j = 0; j < ny; j++)
            {
                array[i][j] = array[nz+i-1][j]; // Copying top rows to bottom ghost cells.
                array[nz+nz_ghost+i][j] = array[nz_ghost+i+1][j]; // Copying bottom rows to top ghost cells.
            }
        }
        for (int j = 0; j < ny; j++)
        {
            array[nz+nz_ghost][j] = array[nz_ghost][j]; // Making sure the boundary on the top is the same as the bottom.
        }
    ...
    {
        // Send to bottom
        MPI_Send(&array[nz-1][0], n_send, MPI_FLOAT_P, 0, 0, MPI_COMM_WORLD);
        // Recieve from bottom
        MPI_Recv(&array[nz+nz_ghost][0], n_send, MPI_FLOAT_P, 0, 1, MPI_COMM_WORLD, &status);
    }
    // Send and recieve to/from top
    if (!has_neighbor_below)
    {
        // Recieve from top
        MPI_Recv(&array[0][0], n_send, MPI_FLOAT_P, size-1, 0, MPI_COMM_WORLD, &status);
        // Send to top
        MPI_Send(&array[nz_ghost+1][0], n_send, MPI_FLOAT_P, size-1, 1, MPI_COMM_WORLD);
    }
    // Lastly we make sure the boundary on the top is the same as the bottom.
    if (!has_neighbor_below)
    {
        MPI_Send(&array[nz_ghost][0], ny, MPI_FLOAT_P, size-1, 0, MPI_COMM_WORLD);
    }
    if (!has_neighbor_above)
    {
        MPI_Recv(&array[nz+nz_ghost-1][0], ny, MPI_FLOAT_P, 0, 0, MPI_COMM_WORLD, &status);
    }
}