...
void save_info(struct MpiInfo *mpi_info)
{
    if (mpi_info->rank == 0)
    {
        ...
        int ny = NY;
        int nz = NZ;
        FLOAT_P gs_tol = GS_TOL;
        int gs_max_iter = GS_MAX_ITER;
        ...
        create_write_dataset(group_parameters, "GS_TOL", H5_FLOAT_P, dataspace_parameters, &gs_tol, "Gauss-Seidel tolerance");
        create_write_dataset(group_parameters, "GS_MAX_ITER", H5T_NATIVE_INT, dataspace_parameters, &gs_max_iter, "Gauss-Seidel max iterations");
