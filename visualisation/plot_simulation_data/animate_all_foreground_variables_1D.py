from .plot_all_foreground_variables_1D import plot_all_foreground_variables_1D
...
def animate_all_foreground_variables_1D(folder, save=False, save_name=None, fps=4, save_interval=10, anim_interval=250, **kwargs):

    fig = plt.figure(figsize=(16, 9))  # Create a figure
    num_snaps = get_num_snaps(folder)

    def init_animation():
        plot_all_foreground_variables_1D(folder, fig, 0, **kwargs)
    ...
    def update_animation(snap_number):
        ...
        fig.clear()
        plot_all_foreground_variables_1D(folder, fig, snap_number, **kwargs)
